...
//    cs_1_0 = CapacitiveSensor(1,0);
    ...
    if(wear >= 0){ // check for error
        if(wear < 400) unuse += 1; // increment unuse count
        else use += 1; // increment use count
        
        Bean.sleep(0x124F80); // time waiting for next count check up
    }
    ...
    if(command == 'P'){ // command for print
      ...
      Serial.print(message);
      message = "Use = ";
      message += (use*20); // use count * 20 minutes
      message += " minutes";
      Serial.print(message);
      message = "Unuse = ";
      message += (unuse*20); // unuse count * 20 minutes
      message += " minutes";
      Serial.print(message);
